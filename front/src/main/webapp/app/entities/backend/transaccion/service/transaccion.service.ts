...
type RestOf<T extends ITransaccion | NewTransaccion> = Omit<T, 'fecha'> & {
  fecha?: string | null;
};
...
export class TransaccionService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/transaccions', 'backend');
  ...
  protected convertDateFromClient<T extends ITransaccion | NewTransaccion | PartialUpdateTransaccion>(transaccion: T): RestOf<T> {
    return {
      ...transaccion,
      fecha: transaccion.fecha?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransaccion: RestTransaccion): ITransaccion {
    return {
      ...restTransaccion,
      fecha: restTransaccion.fecha ? dayjs(restTransaccion.fecha) : undefined,
    };
  }
